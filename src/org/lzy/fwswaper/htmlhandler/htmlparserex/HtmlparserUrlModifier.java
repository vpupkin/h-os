...
public class HtmlparserUrlModifier extends UrlModifyingVisitor {
	private static final Logger log = Logger
			.getLogger(HtmlparserUrlModifier.class.getName());
		...
		if (!HtmlHandlerHelper.isHttpLikeProtocolUrl(base))
			throw new IllegalArgumentException(String.format(
					"Base url argument '%s' is not http like protocol. "
							+ "They are not prefix with '%s' or '%s'",
					this.base.toString(), HtmlHandlerHelper.HttpProtocol,
					HtmlHandlerHelper.HttpsProtocol));
	...
	public void visitTag(Tag tag) {
		try {
			if (tag instanceof LinkTag) {
				...
				log.info(String.format("Found link: '%s' => '%s'.", link
						.getLinkText(), link.extractLink()));
				...
				log.info(String.format("Found head link: '%s' => '%s'.", link
						.getLinkText(), link.getLink()));
				...
				if ((src != null) && (src.length() > 0)) {
					...
					log.info(String.format("Found script: '%s' => '%s'.",
							script.getLanguage(), src));
				...
				log.info(String.format("Found image => '%s'.", img
						.getImageURL()));
				...
				log.info(String.format("Found frame: '%s' => '%s'.", frame
						.getText(), frame.getFrameLocation()));
				...
				log.info(String.format("Found form: '%s' => (%s) '%s'.", form
						.getFormName(), form.getFormMethod(), form
						.extractFormLocn()));
			...
			log.warning(String.format(
					"Modify url failed. Exception message: '%s'.",
